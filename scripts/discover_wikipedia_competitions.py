...
YOUTH = re.compile(r"high school|secondary school|school students?|pre.?college|teen(?:ager)?s?|youth|young people|under.?18|grades?\s+[6-9]|students? aged", re.I)
...
def category_pages(category: str) -> list[dict]:
    pages: list[dict] = []
    continuation = ""
    while True:
        params = {"action": "query", "list": "categorymembers", "cmtitle": f"Category:{category}", "cmnamespace": "0", "cmlimit": "500"}
        if continuation: params["cmcontinue"] = continuation
        payload = api(params)
        pages.extend(payload.get("query", {}).get("categorymembers", []))
        continuation = payload.get("continue", {}).get("cmcontinue", "")
        if not continuation: return pages
...
def main() -> None:
    ...
    for pageid, page in sorted(detail.items(), key=lambda item: item[1].get("title", "")):
        ...
        if not title or not description or not YOUTH.search(f"{title} {description}"):
            continue
        records.append({
